...
//        dd(Auth::user()->mandante);
        ...
        else $order = $order->orderBy('posted_at', 'desc' );
...
//            'cache' => $this->cache,
	...
	public function create($host, SharedStat $sharedStat)
	{
        ...
        return view('orders.create', compact('host'))->with([
            'products' => Product::all(),
//            'partners' => Partner::all(),
            'partners' => $sharedStat->where(['status'=>'ativado'])->first()->partners()->get(),
            'order' => new Order,
            'currencies' => SharedCurrency::lists('nome_universal','id'),
            'partner_list' => [''=>''] + (($partners = $sharedStat->where(['status'=>'ativado'])->first()->partners()->orderBy('nome', 'asc' )->get())?$partners->lists('nome','id'):[]),
//            'partner_list' => [''=>''] + Partner::lists('nome','id'),
            'order_types' => SharedOrderType::lists('descricao','id'),
            'order_payment' => SharedOrderPayment::lists('descricao','id'),
            'status' => SharedStat::lists('descricao','id'),
            'viewItemOrderForm' => view('orders.partials.itemOrderForm')->with([
                'product_list' => [''=>''] + (($products = $sharedStat->where(['status'=>'ativado'])->first()->products()->orderBy('nome', 'asc' )->get())?$products->lists('nome','id'):[]),
//                'product_list' => [''=>''] + Product::lists('nome','id'),
                'costs' => [''=>''] + (($costs = CostAllocate::get())? $costs->lists('cost','id'):[]),
                'currencies' => SharedCurrency::lists('nome_universal','id'),
//                'itemCount' => $this->itemCount,
                'itemOrders' => $itemOrders,
            ]),
            'viewPagamentoForm' => view('orders.partials.pagamentoForm'),
            'viewConsumoForm' =>  view('orders.partials.consumoForm'),
            'viewTransporteForm' =>  view('orders.partials.transporteForm')->with([
                'order' => new Order,
                'addresses' => [''=>''] + (($addresses = Address::get())? $addresses->lists('endereco','id'):[]),
            ]),
            'viewAnexosForm' =>  view('orders.partials.anexosForm'),
        ]);
	}
...
//        flash()->success(trans('delivery.flash.pedidoAdd', ['pedido' => $addedOrder->id]));
	...
	{
//        dd($order->status()->where('status', 'aberto')->get());
        return view('orders.edit', compact('host', 'order'))->with([
            'products' => Product::all(),
            'partners' => $sharedStat->where(['status'=>'ativado'])->first()->partners()->get(),
//            'order' => new Order,
            'currencies' => SharedCurrency::lists('nome_universal','id'),
            ...
            'viewPagamentoForm' => view('orders.partials.pagamentoForm'),
            'viewConsumoForm' =>  view('orders.partials.consumoForm'),
            'viewTransporteForm' =>  view('orders.partials.transporteForm', compact('order'))->with([
                'addresses' => [''=>''] + (($addresses = Address::get())? $addresses->lists('endereco','id'):[]),
            ]),
            'viewAnexosForm' =>  view('orders.partials.anexosForm'),
        ]);

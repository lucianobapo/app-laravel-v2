...
@section('content')
    ...
    {!! Form::model($order, ['method'=>'PATCH','url'=>route('orders.update', [$host,$order->id]),
        ...
        @include ('orders.partials.form', ['submitButtonText'=>trans('order.update.orderBtn'),
...
//                    '<span><img src="vendor/images/flags/' + state.element.value.toLowerCase() + '.png" class="img-flag" /> ' + state.text + '</span>'
        ...
        app.controller('myCtrl', function($scope) {
            ...
            @foreach($partners as $partner)
                partners [{{ $partner->id }}] = "{{ is_null($address = Address::where(['partner_id'=>$partner->id])->first())?'':$address->endereco }}";
            @endforeach
        });
    </script>
